...
class Pass:
    def __init__(self,id):
        ...
        self.login.iconbitmap("photo//1458264596_authorisation_lock_padlock_safe_password_privacy_security_icon-icons.com_55333.ico")
...
#-----------------------------login app---------------------- 
    def inapp(self):
        ...
        query = f"SELECT password FROM teacher_table WHERE id = %s LIMIT 1"
        cur.execute(query, (self.id,))
        u = cur.fetchone()
        old = u[0]
        
        if str(old) == str(current) :
             b = "UPDATE teacher_table SET password = %s WHERE id = %s"
