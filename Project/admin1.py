...
class Student:
    def __init__(self) :
        self.stu= Toplevel()
        self.stu.geometry("1350x695+1+1")
        ...
        #--------------------------refreshButtom------------------------------
        #----------------------------refresh----------------------------
        self.pic=Image.open("photo//refresh.png")#======================refresh pic==================
...
#refresh
    ...
    def curces(self):
            ...
            name=Label(fr1,text="اسم الطالب الرباعي",fg="black",bg="white",width=30)
            name.pack()
            nametext=Entry(fr1,justify="left",width=30,bd='2',textvariable=self.namevar)
            nametext.pack()
            sub1=Label(fr1,text="Subject1",width=30,fg="black",bg="white")
            sub1.pack()
            sub1text= Entry(fr1,justify='left' , width=30,bd='2', fg= 'black', textvariable=self.sub1var)
            sub1text.pack()
    ...
    def add_student (self):
        ...
        try:
          ...
          cur1.execute('insert into students (password) values (%s)',('0000'))
    ...
    def mycursor_sub(self, ev):  # ev is your event that has everything
      info = self.curse.focus()
      contents = self.curse.item(info)
      row = contents['values']

    # Check if row contains enough elements to avoid IndexError
      if len(row) > 10:  # Check if row has at least 11 elements
        self.idvar.set(row[0])  # Set idvar
        self.namevar.set(row[1])  # Set namevar
        self.sub1var.set(row[2])  # Set sub1var
        self.sub2var.set(row[3])  # Set sub2var
        self.sub3var.set(row[4])  # Set sub3var
        self.sub4var.set(row[5])  # Set sub4var
        self.sub5var.set(row[6])  # Set sub5var
        self.sub6var.set(row[7])  # Set sub6var
        self.scorevar.set(row[9])  # Set scorevar
        self.semvar.set(row[10])  # Set semvar
      else:
        print("Row does not contain enough data!")
        # Optionally, you can handle the case when the row doesn't contain enough values
        # For example, set some default values or show a warning message
        self.idvar.set("N/A")
        self.namevar.set("N/A")
        self.sub1var.set("N/A")
        self.sub2var.set("N/A")
        self.sub3var.set("N/A")
        self.sub4var.set("N/A")
        self.sub5var.set("N/A")
        self.sub6var.set("N/A")
        self.scorevar.set("N/A")
        self.semvar.set("N/A")
    ...
    def search(self):
        ...
        do=con.cursor()
        try:
                do.execute("select id , name , date , nation , watani , gender , phone ,city, address from students where " +
                str(self.sby.get())+" LIKE '%" +str(self.svar.get())+"%'")#----link combo with entry-----
                rows= do.fetchall()#-----bring all data-----
                if len(rows)!=0:
                   self.student.delete(*self.student.get_children())
                else:
                    messagebox.askretrycancel("error","the student is not found! ")
                for row in rows:#---------loop for bring it up to the table-------
                   self.student.insert('',END,values=row)
        except:
